...
class PyVQFJosef:
    ...
    def updateAcc(self, acc: np.ndarray) -> None:
        ...
        accCorrQuat = np.array([1, 0.5*accEarth[1], -0.5*accEarth[0], 0], float)
        
        self.coefs.append(list(accEarth.copy()))
    ...
    def updateMag(self, mag: np.ndarray) -> None:
        ...
        mstep = self._params.tauMag * 0.01 * 0.0215351
        m_step_dir =-mstep if magEarth[0] < 0 else mstep       
        
        # Correction quaternion
        q_cor = np.array([1, 0, 0, m_step_dir])
        
        self._state.accQuat = self.quatMultiply(q_cor, self._state.accQuat)
    ...
    def filterVec(x: np.ndarray,
        ...
        if math.isnan(state[0, 0]):  # initialization phase
            ...
            if state[0, 1]*Ts >= tau:
                for i in range(N):
                    state[:, i] = PyVQFJosef.filterInitialState(out[i], b, a)
            return out

        return PyVQFJosef.filterStep(x, b, a, state)
